...
def enhanced_network_scan():
    ...
    try:
        ...
        data = request.get_json() or {}
        network_range = data.get('network_range', '192.168.1.0/24')
        # Try several method names for enhanced discovery and fall back
        discovered_devices = []
        try:
            if hasattr(device_manager, 'enhanced_network_discovery'):
                discovered_devices = device_manager.enhanced_network_discovery(network_range)
            elif hasattr(device_manager, 'enhanced_scan'):
                discovered_devices = device_manager.enhanced_scan(network_range)
            else:
                # No enhanced discovery available; fall back to basic discovery
                if hasattr(device_manager, 'discover_network_devices'):
                    discovered_devices = device_manager.discover_network_devices(network_range)
                elif hasattr(device_manager, 'scan_arp_table'):
                    discovered_devices = device_manager.scan_arp_table()
            ...
            logger.error(f"Enhanced discovery implementation error: {inner_e}")
            # Attempt ARP fallback
            try:
                if hasattr(device_manager, 'scan_arp_table'):
                    discovered_devices = device_manager.scan_arp_table()
            except Exception:
        ...
        return jsonify({'error': 'Enhanced network scan failed'}), 500
