...
async function getArpTable() {
    try {
        ...
        if (data.success && (data.arp_entries || data.devices || data.discovered_devices)) {
            ...
            document.getElementById('discoveryResults').innerHTML = `❌ Error: ${data.error || data.message || 'No devices found'}`;
        }
    } catch (error) {
        document.getElementById('discoveryResults').innerHTML = `❌ Error: ${error.message}`;
    }
}
...
async function pingSingleDevice() {
    ...
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('discoveryResults').innerHTML = resultHtml;
