...
def dashboard():
    ...
    try:
        ...
        # Format in English, the template JS will still update live with Afrikaans names
        server_date_iso = server_now.isoformat()
        doctor_name = 'Dr. Stoyanov'
        
        logger.info(f"Rendering dashboard with date: {server_date_iso}, doctor: {doctor_name}")
        return render_template('dashboard_sa.html', server_date_iso=server_date_iso, doctor_name=doctor_name)
...
def templates():
    ...
    try:
        from .template_routes import render_templates
