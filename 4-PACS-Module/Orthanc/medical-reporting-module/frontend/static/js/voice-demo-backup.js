...
class SAVoiceDemo {
    ...
    async startVoiceSession() {
        try {
            ...
            if (response.ok) {
                ...
                console.log('🎤 Voice session started:', this.sessionId || '(no id returned)');
            }
        } catch (error) {
    ...
    async processCompleteRecording() {
        ...
        try {
            console.log(`🎵 Processing complete recording: ${this.audioChunks.length} chunks`);
            ...
            console.log(`📦 Complete audio: ${completeAudioBlob.size} bytes`);
    ...
    async transcribeCompleteAudio(audioBlob) {
        try {
            console.log(`🎵 Transcribing complete audio: ${audioBlob.size} bytes`);
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (result.success && result.transcription) {
                        ...
                        console.warn('❌ Transcription failed:', result.error || 'No transcription returned');
    ...
    loadTemplate(templateType) {
        ...
        console.log(`📋 ${templateType} template loaded`);
    }
    ...
    async checkForVoiceShortcuts(audioBlob) {
        ...
        try {
            ...
            const response = await fetch('/api/voice/shortcuts/demo/match', {
                method: 'POST',
                body: formData
            });
            ...
            if (response.ok) {
                ...
                if (result.success && result.matched_shortcut) {
                    ...
                    console.log(`🎯 Voice shortcut triggered: ${shortcut.name}`);
    ...
    async executeVoiceShortcut(shortcut) {
        console.log(`🚀 Executing voice shortcut: ${shortcut.name}`);
                ...
                <div class="flex justify-between items-start">
                    <div class="flex-1">
                        ...
                        <div class="text-xs text-gray-500">
                            ...
                            ${shortcut.usage_count ? ` • Used ${shortcut.usage_count} times` : ''}
                        </div>
                    </div>
                    <div class="flex space-x-2">
    ...
    async loadUserShortcuts() {
        try {
            const response = await fetch(`/api/voice/shortcuts/demo?user_id=${this.userId}`);
            const data = await response.json();
