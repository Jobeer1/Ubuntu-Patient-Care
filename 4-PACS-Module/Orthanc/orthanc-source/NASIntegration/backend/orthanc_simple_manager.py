...
class SimpleOrthancManager:
    ...
    def start_orthanc(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if Orthanc executable exists
            orthanc_exe = 'Orthanc.exe' if os.name == 'nt' else 'orthanc'
            orthanc_path = None
            
            # Try to find Orthanc in common locations
            common_paths = [
                orthanc_exe,  # In PATH
                './Orthanc.exe',  # Current directory
                '../Orthanc.exe',  # Parent directory
                'C:/Program Files/Orthanc Server/Orthanc.exe',  # Windows default installation
                'C:/Orthanc/Orthanc.exe',  # Alternative Windows location
                '/usr/bin/orthanc',  # Linux default
                '/usr/local/bin/orthanc'  # Linux alternative
            ]
            ...
            if not orthanc_path:
                return {
                    'success': False, 
                    'message': f'Orthanc executable not found. Please install Orthanc server first.',
                    'installation_help': {
                        'windows': 'Download from: https://www.orthanc-server.com/download-windows.php',
                        'linux': 'sudo apt install orthanc',
            ...
            if self.is_orthanc_running():
                ...
                return {'success': False, 'message': 'Failed to start Orthanc - check logs for details'}
