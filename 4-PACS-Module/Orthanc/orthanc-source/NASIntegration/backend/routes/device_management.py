...
class DeviceManager:
    ...
    def remove_nas_by_ip(self, ip_address):
        """Remove any NAS configuration entries matching the IP address."""
        try:
            with open(self.nas_config_file, 'r') as f:
                config = json.load(f)

            removed = []
            to_delete = [k for k, v in config.get('nas_devices', {}).items() if v.get('ip_address') == ip_address]
            for k in to_delete:
                removed.append(k)
                del config['nas_devices'][k]

            if removed:
                config['last_updated'] = datetime.now().isoformat()
                with open(self.nas_config_file, 'w') as f:
                    json.dump(config, f, indent=2)
                logger.info(f"Removed NAS configurations for {ip_address}: {removed}")
                return { 'success': True, 'removed': removed }
            else:
                return { 'success': False, 'error': 'No NAS configuration found for this IP' }
        except Exception as e:
            logger.error(f"Error removing NAS by IP {ip_address}: {e}")
