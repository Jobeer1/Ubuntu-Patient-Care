"""Authentication routes - SSO with Google & Microsoft OAuth"""
...
async def auth_microsoft_callback(request: Request, response: Response, db: Session = Depends(get_db)):
    ...
    try:
        logger.info("Microsoft callback received")
        ...
        if "redirect_uri" in error_detail.lower():
            error_detail = "REDIRECT_URI_MISMATCH: Check Azure Portal redirect URI configuration"
